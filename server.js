...
async function webSearch(query) {
  // Primary: Serper.dev (Google results, fast and accurate)
  if (SEARCH_API_KEY) {
    try {
      const r = await fetch("https://google.serper.dev/search", {
        method: "POST",
        headers: { "X-API-KEY": SEARCH_API_KEY, "Content-Type": "application/json" },
        body: JSON.stringify({ q: query, num: 5, gl: "lk", hl: "en" }),
      });
      const d = await r.json();
      let result = "";
      if (d.answerBox?.answer) result += d.answerBox.answer + " ";
      if (d.answerBox?.snippet) result += d.answerBox.snippet + " ";
      if (d.answerBox?.snippetHighlighted?.length) result += d.answerBox.snippetHighlighted.join(" ") + " ";
      if (d.knowledgeGraph?.description) result += d.knowledgeGraph.description + " ";
      if (!result && d.organic?.length) result = d.organic.slice(0,3).map(o=>o.snippet).filter(Boolean).join(" ");
      if (!result && d.topStories?.length) result = d.topStories.slice(0,2).map(s=>s.title).join(". ");
      if (result.trim()) return { success: true, result: result.trim() };
  ...
  return { success: false, result: "I could not find that information right now." };
}
